...
class Scratch3NewBlocks {　　//とりあえず初期化してる
            ...
            blocks: [
                {
                    ...
                    text: 'log [TEXT]',// ブロックの名前。[と]の間に英数字を入れると引数になる。って書いたあったけどよくわからんので聞いてみる
                    arguments: {
                        TEXT: {       //上で指定した引数
    ...
    writeLog (args) {　　　//オペコードはオペレーションコードで命令　argsは引数　上で定義してる変数の中身をもってこれる
        const text = Cast.toString(args.TEXT);　　//引数　tostringは文字列にtoNumberは数値に
        log.log(text);
    }
     ...
     */
    getBrowser (args) {
        const text = Cast.toNumber(args.test)*2;
        ///let　定数
        ///const 変数
